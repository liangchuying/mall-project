...
| 用例编号 | 测试场景 | 预期结果 |
|----------|----------|----------|
| TC-008 | 正常登录 | 返回 code 200，包含 token 和用户信息 |
| TC-009 | 用户名不存在 | 返回 code 500，提示"用户不存在" |
| TC-010 | 密码错误 | 返回 code 500，提示"密码错误" |
| TC-011 | 账号被禁用 | 返回 code 500，提示"账号已被禁用" |
| TC-012 | 用户名为空 | 返回参数校验错误 |
| TC-013 | 密码为空 | 返回参数校验错误 |
